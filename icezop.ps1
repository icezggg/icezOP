...
# ══════════════════ 2. TEMA (COLORES, GLIFOS, FUENTES) ══════════════════
 $Script:C = @{
    ...
    Acc    = [System.Drawing.Color]::FromArgb(139, 92, 246)   # #8B5CF6
 ...
 $Script:G = @{
    Menu = [string][char]0xE700; Home = [string][char]0xE80F; Apps = [string][char]0xE71D
    Fix  = [string][char]0xE90F; Drv  = [string][char]0xE721; Gear = [string][char]0xE713
    Play = [string][char]0xE768; X    = [string][char]0xE8BB; Min  = [string][char]0xE921
}

 $Script:GlyphFont = 'Segoe MDL2 Assets'
try {
    $ff = New-Object System.Drawing.FontFamily('Segoe Fluent Icons')
    $Script:GlyphFont = 'Segoe Fluent Icons'; $ff.Dispose()
} catch {}

# ══════════════════ 3. ESTADO GLOBAL ══════════════════
 $Script:AppSel   = @{}   # ID de app  -> $true/$false (selección persistente)
...
# Canal de comunicación UI <-> Runspace (thread-safe)
...
namespace IcezOP
{
    ...
    public class CardPanel : Panel
    {
        ...
        public CardPanel()
        {
            ...
            radius = 16; fill = Color.FromArgb(26, 26, 34); border = Color.FromArgb(42, 42, 53);
        }
    ...
    public class IcezCheckBox : Control
    {
        ...
        private Color top; private Color bottom;
        public bool Checked
        {
        ...
        public IcezCheckBox()
        {
            ...
            top = Color.FromArgb(167, 139, 250); bottom = Color.FromArgb(109, 40, 217);
            Height = 30;
        }
    ...
    public class GradientButton : Control
    {
        ...
        private Color cTop; private Color cBottom; private string glyph; private string gfont; private int radius;
        ...
        public GradientButton()
        {
            ...
            BackColor = Color.Transparent;
            Cursor = Cursors.Hand;
            cTop = Color.FromArgb(139, 92, 246); cBottom = Color.FromArgb(109, 40, 217);
            ...
            using (GraphicsPath p = Gfx.Round(r, radius))
            {
                if (Enabled)
                {
                    ...
                    using (LinearGradientBrush lb = new LinearGradientBrush(r, cTop, cBottom, LinearGradientMode.Vertical))
                        g.FillPath(lb, p);
                    if (isHover && !isDown) { using (SolidBrush hb = new SolidBrush(Color.FromArgb(28, 255, 255, 255))) g.FillPath(hb, p); }
                    if (isDown) { using (SolidBrush db = new SolidBrush(Color.FromArgb(45, 0, 0, 0))) g.FillPath(db, p); }
                }
                else
                {
                    using (SolidBrush nb = new SolidBrush(Color.FromArgb(56, 56, 70))) g.FillPath(nb, p);
                }
            }
    ...
    public class GhostButton : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (GraphicsPath p = Gfx.Round(new RectangleF(0.5f, 0.5f, Width - 1f, Height - 1f), 10f))
            {
                if (isHover)
                {
                    using (SolidBrush b = new SolidBrush(Color.FromArgb(26, 139, 92, 246))) g.FillPath(b, p);
                }
    ...
    public class CategoryTile : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                using (SolidBrush b = new SolidBrush(hov ? Color.FromArgb(35, 35, 49) : Color.FromArgb(27, 27, 35))) g.FillPath(b, p);
                using (Pen pen = new Pen(hov ? Color.FromArgb(90, 139, 92, 246) : Color.FromArgb(42, 42, 53), 1f)) g.DrawPath(pen, p);
            }
            float ms = 40f;
...
function New-Label {
    param([string]$Text, [int]$X, [int]$Y, [int]$W, [int]$H, [single]$Size = 10, $Color = $C.Text, [switch]$Bold, [string]$Family = 'Segoe UI')
...
# ══════════════════ 6. WORKERS (código que corre en Runspaces) ══════════════════
 $Script:WorkerCode = @'
param($sync, $cfg)
...
for ($i = 0; $i -lt $total; $i++) {
        ...
        switch ($t.Kind) {
            ...
            'restore' {
                try {
                    ...
                    $ok = $true
                } catch { Log ('  !  No se pudo crear el punto de restauracion: ' + $_.Exception.Message) }
            }
            ...
            'wingetboot' {
                try {
                    ...
                    else { Log '  X   Winget instalado pero no disponible en PATH. Reinicia e intenta de nuevo.' }
                } catch { Log ('  X   No se pudo instalar Winget: ' + $_.Exception.Message) }
            }

            'app' {
                if (-not $sync.Winget) { Log '  X   Winget no disponible, se omite'; break }
                $wg = $sync.Winget
                ...
                for ($try = 1; $try -le $maxTry; $try++) {
                    ...
                    if ($LASTEXITCODE -eq 0) { Log '  OK  Ya estaba instalado, se omite'; $ok = $true; break }
                    if ($try -eq $maxTry) { Log ('  X   Fallo (codigo de salida ' + $code + ')') }
                }
            }
            ...
            'drivers' {
                try {
                    ...
                    $sea.Online = $true
                    Log '  ..  Buscando actualizaciones de controladores'
                    $res = $sea.Search("IsInstalled=0 and Type='Driver'")
                    $coll = New-Object -ComObject Microsoft.Update.UpdateColl
                    foreach ($u in $res.Updates) {
                        if ($t.Titles -contains [string]$u.Title) { [void]$coll.Add($u) }
                    }
                    if ($coll.Count -eq 0) { Log '  !  Las actualizaciones seleccionadas ya no estan disponibles'; break }
                    $dl = $sess.CreateUpdateDownloader()
                    $dl.Updates = $coll
                    $null = $dl.Download()
                    Log '  ..  Descarga finalizada, instalando'
    ...
    } catch { Log ('  X   Error inesperado: ' + $_.Exception.Message) }
...
# Logo centrado "icez" + "OP" en Segoe Script
 ...
 $gTmp.Dispose()
 $x0 = [int]((1160 - ($w1 + $w2)) / 2)
 $logo1 = New-Label 'icez' $x0 10 ($w1 + 10) 52 22 $C.Text -Family 'Segoe Script'
 $logo1.Font = $fLogo
 $logo2 = New-Label 'OP' ($x0 + $w1) 10 ($w2 + 10) 52 22 $C.AccL -Family 'Segoe Script'
 $logo2.Font = $fLogo
 $header.Controls.Add($logo1); $header.Controls.Add($logo2)

 $btnMin = New-Object IcezOP.GlyphButton
 $btnMin.Location = Pt (1160 - 96) 19
 $btnMin.Glyph = $G.Min; $btnMin.GlyphFont = $Script:GlyphFont
 $btnMin.Add_Click({ $Script:Form.WindowState = 'Minimized' })
 $btnClose = New-Object IcezOP.GlyphButton
 $btnClose.Location = Pt (1160 - 52) 19
 $btnClose.Glyph = $G.X; $btnClose.GlyphFont = $Script:GlyphFont
...
# Separador de 1px bajo el header
 ...
 $sep.Dock = 'Top'; $sep.Height = 1; $sep.BackColor = $C.Border

# ── Barra inferior (Dock Bottom) ─────────────────────────────────────
 $bottom = New-Object System.Windows.Forms.Panel
 $bottom.Dock = 'Bottom'; $bottom.Height = 64; $bottom.BackColor = $C.BgAlt
 $bline = New-Object System.Windows.Forms.Panel
 $bline.Dock = 'Top'; $bline.Height = 1; $bline.BackColor = $C.Border
...
New-NavItem $G.Home 'Inicio'          'home' 20
New-NavItem $G.Apps 'Aplicaciones'    'apps' 68
New-NavItem $G.Fix  'Tweaks'          'tweaks'116
New-NavItem $G.Drv  'Controladores'   'drivers'164
New-NavItem $G.Gear 'Ajustes'         'settings'212

# ── Host de contenido (Dock Fill) ────────────────────────────────────
 $content = New-Object System.Windows.Forms.Panel
 $content.Dock = 'Fill'; $content.BackColor = $C.Bg
 $Script:Pages = @{}

function New-Page([string]$Tag) {
    $p = New-Object System.Windows.Forms.Panel
    $p.Dock = 'Fill'; $p.BackColor = $C.Bg
    $p.Visible = $false
...
function Switch-Page {
    param([string]$Tag)
    foreach ($k in $Script:Pages.Keys) { $Script:Pages[$k].Visible = ($k -eq $Tag); if ($k -eq $Tag) { $Script:Pages[$k].BringToFront() } }
    foreach ($n in $Script:NavList) { $n.IsActive = ([string]$n.Tag -eq $Tag) }
}

# ══════════ PÁGINA: INICIO ══════════
 $pHome = New-Page 'home'
 $pHome.Controls.Add((New-Label 'Inicio' 24 20 400 34 15 $C.Text -Bold))
 $pHome.Controls.Add((New-Label 'El primer programa que ejecutas después de formatear tu PC.' 24 52 600 22 9.5 $C.Sub))

 $hero = New-Card 24 82 892 128
 $hero.Controls.Add((New-Label 'Bienvenido a icezOP' 24 16 400 28 13.5 $C.Text -Bold))
 $hero.Controls.Add((New-Label 'Explorá las categorías, marcá lo que quieras instalar u optimizar, y pulsá EJECUTAR.' 24 46 620 20 9 $C.Sub))
 $btnRec = New-Object IcezOP.GradientButton
 $btnRec.Location = Pt 24 80; $btnRec.Size = Sz 230 36
 ...
 $btnClear.Location = Pt 268 80; $btnClear.Size = Sz 180 36
 $btnClear.Text = 'LIMPIAR SELECCIÓN'
 $btnClear.Add_Click({ $Script:AppSel = @{}; $Script:TweakSel = @{}; Update-HomeUI })
 $hero.Controls.Add($btnClear)
 $pHome.Controls.Add($hero)

 $stats = @(
    @{ N = 'Apps seleccionadas'; V = 0 },
    @{ N = 'Tweaks seleccionados'; V = 0 },
    @{ N = 'Tareas en cola'; V = 0 }
)
 $statRefs = @()
 $sx = 24
foreach ($s in $stats) {
    $c = New-Card $sx 226 289 88
    $num = New-Label '0' 20 10 120 40 20 $C.AccL -Bold
    $cap = New-Label $s.N 20 52 240 20 8.75 $C.Sub
    $c.Controls.Add($num); $c.Controls.Add($cap)
 ...
 $how.Controls.Add((New-Label '¿Cómo funciona?' 24 14 300 24 11 $C.Text -Bold))
 $how.Controls.Add((New-Label '1 · Abrí una categoría y marcá apps (instalación silenciosa vía Winget) o tweaks (registro, servicios, red...).' 24 42 840 18 8.75 $C.Sub))
 $how.Controls.Add((New-Label '2 · Revisá los controladores pendientes en la sección Controladores.' 24 62 840 18 8.75 $C.Sub))
 $how.Controls.Add((New-Label '3 · Pulsá EJECUTAR. Todo se procesa en cola sin congelar la interfaz.' 24 82 840 18 8.75 $C.Sub))
 $pHome.Controls.Add($how)

# ══════════ PÁGINA: APLICACIONES ══════════
 $pApps = New-Page 'apps'
 $pApps.Controls.Add((New-Label 'Aplicaciones' 24 20 400 34 15 $C.Text -Bold))
 $pApps.Controls.Add((New-Label 'Instalación oficial y silenciosa con Winget. El catálogo vive en apps.json — agregá apps sin tocar código.' 24 52 700 22 9.5 $C.Sub))
 $appFlow = New-Object System.Windows.Forms.FlowLayoutPanel
 $appFlow.Location = Pt 24 82
 $appFlow.Size = Sz 892 490
 $appFlow.BackColor = $C.Bg
...
# ══════════ PÁGINA: TWEAKS ══════════
 ...
 $pTweaks.Controls.Add((New-Label 'Tweaks y Optimización' 24 20 500 34 15 $C.Text -Bold))
 $pTweaks.Controls.Add((New-Label 'Ajustes de registro, servicios, energía, red y rendimiento. El catálogo vive en tweaks.json.' 24 52 700 22 9.5 $C.Sub))
 $tweakFlow = New-Object System.Windows.Forms.FlowLayoutPanel
 $tweakFlow.Location = Pt 24 82
 $tweakFlow.Size = Sz 892 490
 $tweakFlow.BackColor = $C.Bg
...
# ══════════ PÁGINA: CONTROLADORES ══════════
 $pDrv = New-Page 'drivers'
 $pDrv.Controls.Add((New-Label 'Controladores' 24 16 400 34 15 $C.Text -Bold))
 $pDrv.Controls.Add((New-Label 'Busca e instala controladores pendientes directamente desde Windows Update.' 24 48 700 22 9.5 $C.Sub))

 $drvCard = New-Card 24 80 892 100
 $drvCard.Controls.Add((New-Label 'Búsqueda de controladores' 20 16 400 24 10.5 $C.Text -Bold))
 $Script:DriverStatusLbl = New-Label 'Nunca se ha buscado. Pulsá el botón para empezar.' 20 44 440 20 9 $C.Sub
 ...
 $Script:DrvSearchBtn.Glyph = $G.Drv
 $Script:DrvSearchBtn.GlyphFont = $Script:GlyphFont
 $drvCard.Controls.Add($Script:DrvSearchBtn)
 $pDrv.Controls.Add($drvCard)

 $pDrv.Controls.Add((New-Label 'Dispositivos con problemas' 24 194 400 24 11 $C.Text -Bold))
 ...
 $Script:DriverProblemsPanel.BackColor = $C.Bg
 $Script:DriverProblemsPanel.AutoScroll = $true
 $pDrv.Controls.Add($Script:DriverProblemsPanel)

 $pDrv.Controls.Add((New-Label 'Actualizaciones disponibles' 24 326 400 24 11 $C.Text -Bold))
 ...
 $Script:DriverListPanel.BackColor = $C.Bg
 ...
 $Script:DrvUpdBtn.Add_Click({
    ...
    [void]$tasks.Add(@{ Kind = 'drivers'; Label = ('Actualizar {0} controlador(es)' -f $titles.Count); Titles = $titles })
 ...
 $Script:DrvSearchBtn.Add_Click({
    ...
    $Script:DriverStatusLbl.Text = 'Buscando actualizaciones de controladores en Windows Update...'
...
function Build-DriverCards {
    ...
    if ($probs.Count -gt 0) {
        ...
        foreach ($p in $probs) {
            $c = New-Object IcezOP.CardPanel
            $c.Location = Pt 0 $y; $c.Size = Sz 874 44
            $c.BorderColor = [System.Drawing.Color]::FromArgb(110, 248, 113, 113)
            $c.Controls.Add((New-Label ('!  ' + $p.Name) 16 11 620 20 9.5 $C.Text))
            $c.Controls.Add((New-Label ('Estado: ' + $p.Status) 646 13 210 18 8.5 $C.Err))
            $Script:DriverProblemsPanel.Controls.Add($c)
            $y += 52
        }
    } else {
        $Script:DriverProblemsPanel.Controls.Add((New-Label 'Ningún dispositivo con problemas.' 4 6 500 20 9 $C.Sub))
    }
    ...
    if ($ups.Count -gt 0) {
        ...
        foreach ($u in $ups) {
            $c = New-Object IcezOP.CardPanel
            $c.Location = Pt 0 $y; $c.Size = Sz 874 58
            $cb = New-Object IcezOP.IcezCheckBox
            $cb.Location = Pt 16 6; $cb.Size = Sz 640 28
            $cb.Text = [string]$u.Title
            $sub = ('v{0} · {1}' -f $u.Version, $u.Date)
            $c.Controls.Add($cb)
            $c.Controls.Add((New-Label $sub 44 32 600 18 8.25 $C.Sub))
            $k = [string]$u.Title
        ...
        $Script:DriverListPanel.Controls.Add((New-Label 'Todo al día. No hay actualizaciones de controladores pendientes.' 4 6 560 20 9 $C.Sub))
    }
    $Script:DriverStatusLbl.Text = ('{0} actualizaciones disponibles · {1} dispositivos con problemas' -f $ups.Count, $probs.Count)
    Update-DriverBtn
}

# ══════════ PÁGINA: AJUSTES ══════════
 $pSet = New-Page 'settings'
 $pSet.Controls.Add((New-Label 'Ajustes' 24 20 400 34 15 $C.Text -Bold))
 $pSet.Controls.Add((New-Label 'Preferencias de ejecución de icezOP.' 24 52 600 22 9.5 $C.Sub))

 $cardExec = New-Card 24 82 892 320
 $cardExec.Controls.Add((New-Label 'Ejecución' 20 16 300 24 11 $C.Text -Bold))

 $cbRestore = New-Object IcezOP.IcezCheckBox
 $cbRestore.Location = Pt 20 52; $cbRestore.Size = Sz 840 30
 $cbRestore.Text = 'Crear punto de restauración antes de aplicar tweaks'
 ...
 $cbSources.Location = Pt 20 88; $cbSources.Size = Sz 840 30
 $cbSources.Text = 'Actualizar orígenes de Winget antes de instalar'
 ...
 $cbWg.Location = Pt 20 124; $cbWg.Size = Sz 840 30
 $cbWg.Text = 'Instalar Winget automáticamente si falta (descarga App Installer)'
 ...
 $cardExec.Controls.Add((New-Label 'Al finalizar la ejecución:' 20 166 300 20 9.5 $C.Sub))
...
[void]$cbFinish.Items.Add('Reiniciar el equipo')
 ...
 $Script:LblRetries = New-Label ('Reintentos de Winget al fallar: {0}' -f $Script:Settings.Retries) 20 236 400 20 9.5 $C.Sub
 $cardExec.Controls.Add($Script:LblRetries)
 $slider = New-Object IcezOP.IcezSlider
 $slider.Location = Pt 20 258; $slider.Size = Sz 300 26
 $slider.Minimum = 0; $slider.Maximum = 3; $slider.Value = $Script:Settings.Retries
...
function Show-CategoryModal {
    ...
    # Overlay oscuro (simula blur/acrylic tapando la ventana principal)
    $ov = New-Object System.Windows.Forms.Form
    $ov.FormBorderStyle = 'None'; $ov.ShowInTaskbar = $false; $ov.StartPosition = 'Manual'
    $ov.Bounds = $Script:Form.Bounds
    $ov.BackColor = [System.Drawing.Color]::FromArgb(8, 8, 12)
    $ov.Opacity = 0.72
    ...
    $modal.FormBorderStyle = 'None'; $modal.ShowInTaskbar = $false; $modal.StartPosition = 'Manual'
    $modal.Size = Sz $mw $mh
    ...
    $counter = New-Label '' ($mw - 250) 22 160 20 9 $C.Sub
    $modal.Controls.Add($counter)
    if ($Category -match '(?i)riesgo|experimental') {
        $modal.Controls.Add((New-Label '! Categoría experimental: algunos ajustes pueden causar inestabilidad.' 28 40 520 18 8.5 $C.Err))
    }

    $btnX = New-Object IcezOP.GlyphButton
    $btnX.Location = Pt ($mw - 46) 14
    $btnX.Glyph = $G.X; $btnX.GlyphFont = $Script:GlyphFont
...
# ══════════════════ 9. MODAL DE EJECUCIÓN + MOTOR ══════════════════
function Start-Run {
    param([System.Collections.ArrayList]$Tasks, [string]$Title = 'Ejecutando selección')

    $sync.Running = $true;  $sync.Done = $false; $sync.Cancel = $false
    $sync.Percent = 0;      $sync.Status = 'Preparando...'
    $sync.OkCount = 0;      $sync.FailCount = 0
    $sync.Log = [System.Collections.Queue]::Synchronized((New-Object System.Collections.Queue))
    $sync.Tasks = $Tasks
    ...
    $Script:RunStatusLbl = New-Label 'Preparando...' 28 62 620 22 9.75 $C.Sub
    ...
    $Script:RunCancel.ForeColor = $C.Err
    ...
    $Script:RunSummary = New-Label '' 28 56 624 64 11 $C.Text -Bold
    ...
    $Script:RunRestart.Add_Click({
        Start-Process 'shutdown.exe' -ArgumentList @('/r', '/t', '5', '/c', 'icezOP - Reiniciando para aplicar los cambios')
        $Script:Form.Close()
    })
    ...
    $Script:RunFinish.Add_Click({
        switch ($Script:Settings.OnFinish) {
            ...
            'restart' { Start-Process 'shutdown.exe' -ArgumentList @('/r', '/t', '5', '/c', 'icezOP - Reiniciando para aplicar los cambios'); $Script:Form.Close() }
    ...
    # Lanzar worker en Runspace (la UI nunca se congela)
 ...
 $Script:ExecBtn.Add_Click({
    ...
    if ($appsSel.Count -gt 0 -and -not $wg) {
        if ($Script:Settings.AutoWinget) {
            [void]$tasks.Add(@{ Kind = 'wingetboot'; Label = 'Instalar Winget (App Installer)' })
        } else {
            [void][System.Windows.Forms.MessageBox]::Show($Script:Form, 'Winget no está disponible. Instálalo desde Microsoft Store ("Instalador de aplicación") o activa la instalación automática en Ajustes.', 'icezOP', 'OK', 'Warning')
            return
        }
    }
    if ($twSel.Count -gt 0 -and $Script:Settings.RestorePoint) { [void]$tasks.Add(@{ Kind = 'restore'; Label = 'Crear punto de restauración' }) }
    if ($appsSel.Count -gt 0 -and $wg -and $Script:Settings.UpdateSources) { [void]$tasks.Add(@{ Kind = 'sources'; Label = 'Actualizar orígenes de Winget' }) }
    ...
    Start-Run -Tasks $tasks -Title 'Ejecutando selección'
})

# ══════════════════ 10. TIMERS (polling + animaciones) ══════════════════
 ...
 $uiTimer.Add_Tick({
    ...
    if ($Script:RunActive) {
        ...
        if ($sync.Done) {
            ...
            try {
                ...
                $Script:RunPs.Dispose(); $Script:RunRs.Dispose()
            } catch {}
    ...
    switch ($sync.DriverPhase) {
        'searching' { if ($Script:DriverStatusLbl) { $Script:DriverStatusLbl.Text = 'Buscando actualizaciones de controladores en Windows Update...' } }
        'done' {
            if (-not $Script:DriverRendered) { $Script:DriverRendered = $true; Build-DriverCards; $sync.DriverPhase = 'idle' }
        }
        'error' {
            if (-not $Script:DriverRendered) {
                $Script:DriverRendered = $true
                $Script:DriverStatusLbl.Text = ('Error en la búsqueda: ' + $sync.DriverError)
                $sync.DriverPhase = 'idle'
            }
        }
    }
